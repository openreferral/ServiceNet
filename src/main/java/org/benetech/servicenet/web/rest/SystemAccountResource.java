...
public class SystemAccountResource {
    ...
    @Timed
    public ResponseEntity<List<SystemAccountDTO>> getAllSystemAccounts(Pageable pageable) {
        log.debug("REST request to get all SystemAccounts");
        Page<SystemAccountDTO> page = systemAccountService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/system-accounts");
