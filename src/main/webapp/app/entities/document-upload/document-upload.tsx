...
import { getEntities, updateEntity } from './document-upload.reducer';
...
export interface IDocumentUploadState extends IPaginationBaseState {
  dropdownOpenTop: boolean;
  dropdownOpenBottom: boolean;
  itemsPerPage: number;
}

export class DocumentUpload extends React.Component<IDocumentUploadProps, IDocumentUploadState> {
  constructor(props) {
    super(props);
  ...
  render() {
    const { documentUploadList, match, totalItems } = this.props;
    return (
      <div>
...
const mapStateToProps = ({ documentUpload }: IRootState) => ({
  ...
  totalItems: documentUpload.totalItems
});
