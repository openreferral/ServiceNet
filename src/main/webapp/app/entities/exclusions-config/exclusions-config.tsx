import React from 'react';
import { connect } from 'react-redux';
import { Link, RouteComponentProps } from 'react-router-dom';
import { Button, Col, Row, Table } from 'reactstrap';
// tslint:disable-next-line:no-unused-variable
import { Translate, ICrudGetAllAction } from 'react-jhipster';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

import { IRootState } from 'app/shared/reducers';
import { getEntities } from './exclusions-config.reducer';
import { IExclusionsConfig } from 'app/shared/model/exclusions-config.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IExclusionsConfigProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export class ExclusionsConfig extends React.Component<IExclusionsConfigProps> {
  componentDidMount() {
    this.props.getEntities();
  }

  render() {
    const { exclusionsConfigList, match } = this.props;
    return (
      <div>
        <h2 id="exclusions-config-heading">
          <Translate contentKey="serviceNetApp.exclusionsConfig.home.title">Exclusions Configs</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="serviceNetApp.exclusionsConfig.home.createLabel">Create new Exclusions Config</Translate>
          </Link>
        </h2>
        <div className="table-responsive">
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="global.field.id">ID</Translate>
                </th>
                <th>
                  <Translate contentKey="serviceNetApp.exclusionsConfig.account">Account</Translate>
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {exclusionsConfigList.map((exclusionsConfig, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${exclusionsConfig.id}`} color="link" size="sm">
                      {exclusionsConfig.id}
                    </Button>
                  </td>
                  <td>
                    {exclusionsConfig.accountId ? (
                      <Link to={`system-account/${exclusionsConfig.accountId}`}>{exclusionsConfig.accountId}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${exclusionsConfig.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${exclusionsConfig.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${exclusionsConfig.id}/delete`} color="danger" size="sm">
                        <FontAwesomeIcon icon="trash" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.delete">Delete</Translate>
                        </span>
                      </Button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </Table>
        </div>
      </div>
    );
  }
}

const mapStateToProps = ({ exclusionsConfig }: IRootState) => ({
  exclusionsConfigList: exclusionsConfig.entities
});

const mapDispatchToProps = {
  getEntities
};

type StateProps = ReturnType<typeof mapStateToProps>;
type DispatchProps = typeof mapDispatchToProps;

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(ExclusionsConfig);
